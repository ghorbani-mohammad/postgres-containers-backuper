...
def backup():
    for container in client.containers.list(filters={"expose": "5432"}):
        ...
        # dump postgres db
        code, result = container.exec_run(cmd=['pg_dumpall', '-c', '-U', 'postgres'], tty=True)
        if code != 0:
            continue
        ...
        s3.meta.client.upload_file(Filename=file_path, Bucket=BUCKET_NAME, Key=file_path.split('/')[-1])
...
def rotate_old_backups():
    ...
    rotate_instance = rotate_backups.RotateBackups(rotation_scheme, prefer_recent=True, strict=False)
    for container in client.containers.list(filters={"expose": "5432"}):
        service_folder = backup_folder / container.name
